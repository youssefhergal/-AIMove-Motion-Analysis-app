...
class SkeletonViewer {
	...
	async createSphereMeshes() {
		...
		const bonesCount = this.globalResult.skeleton.bones.length
	...
	async reCenter() {
		...
		/////////////////////////////////////////////////////////////////////////////
		/////////////////////////////////////////////////////////////////////////////
		const originalRootWorldPos = new THREE.Vector3()
		const originalFirstMeshWorldPos = new THREE.Vector3()

		this.globalResult.skeleton.bones[0].getWorldPosition(originalRootWorldPos)
		this.sphereMeshes.children[0].getWorldPosition(originalFirstMeshWorldPos)
		/////////////////////////////////////////////////////////////////////////////
		/////////////////////////////////////////////////////////////////////////////
		...
		if (childMesh) {
			...
			/////////////////////////////////////////////////////////////////////////
			/////////////////////////////////////////////////////////////////////////
			const newRootWorldPos = new THREE.Vector3()
			const newFirstMeshWorldPos = new THREE.Vector3()

			this.globalResult.skeleton.bones[0].getWorldPosition(newRootWorldPos)
			this.sphereMeshes.children[0].getWorldPosition(newFirstMeshWorldPos)
			/////////////////////////////////////////////////////////////////////////////////////////
			const deltaMesh = new THREE.Vector3().subVectors(
				newFirstMeshWorldPos,
				originalFirstMeshWorldPos
			)

			const deltaRoot = new THREE.Vector3().subVectors(
				newRootWorldPos,
				originalRootWorldPos
			)

			this.offset = new THREE.Vector3().subVectors(deltaMesh, deltaRoot)

			// console.log(
			// 	"Real offset between mesh and root:",
			// 	this.offset,
			// 	totalDistanceY
			// )
			/////////////////////////////////////////////////////////////////////////
			...
			// this.newParent.getWorldPosition(transformedPos)
			...
			// console.log("Actual world offset:", offset)
	...
	async getWorldPositionTimeSeriesByName() {
		...
		if (!firstTrack) {
			console.error("No valid track with time values found.")
			return []
		}

		const times = firstTrack.times
		const allFrames = []
		const boneNames = this.globalResult.skeleton.bones.map((b) => b.name)
		...
		for (let i = 0; i < times.length; i++) {
			...
			const bones = {}
			this.sphereMeshes.children.forEach((sphere, index) => {
				const pos = new THREE.Vector3()
				sphere.getWorldPosition(pos)
				const name = boneNames[index] || `bone_${index}`
				bones[name] = pos.clone()
			})

			allFrames.push({ time, bones })
		}
	...
	getTimeSeries(jointName = "RightArm") {
		...
		for (let i = 0; i < rotationTrack.times.length; i++) {
			...
			// const worldPosition2 = new THREE.Vector3().addVectors(
			// 	boneWorldPos,
			// 	this.offset
			// )
			// const worldPosition = worldPosition2
			// 	.clone()
			// 	.applyEuler(this.rotatedPos)

			// const worldPosition = worldPosition2
			// 	.clone()
			// 	.applyQuaternion(
			// 		new THREE.Quaternion().setFromEuler(this.rotatedPos)
			// 	)
			...
			// positionsZ.push(worldFramesBones()[i].bones[selectedJoint()].z)
			// positions.push(worldFramesBones()[i].bones[selectedJoint()].clone())
	...
	async addListeners() {
		...
		this._onClick = (event) => {
			...
			try {
				...
				console.warn("⚠️ Error during joint hover selection:", err)
			}
		}
	...
	logLeftRightJoints(skeleton) {
		...
		// First pass: Calculate min and max X values
		const positions = skeleton.bones.map((bone) => {
			...
			return { name: bone.name, x: pos.x }
		})

		const rangeX = maxX - minX
		const centerX = minX + rangeX / 2

		const exclusionPercent = 0.01 // e.g., 1% of the range = central area
		const exclusionRange = rangeX * exclusionPercent

		// Second pass: Categorize bones
		positions.forEach(({ name, x }) => {
			if (name === "ENDSITE") return // skip end sites

			if (x < centerX - exclusionRange / 2) {
				right.push(name)
			} else if (x > centerX + exclusionRange / 2) {
				left.push(name)
			}
