...
class SkeletonViewer {
	...
	loadSkeleton(bvhFile) {
		...
		loader.load(bvhFile, (result) => {
			...
			this.setupMixer(this.globalResult) // Setup the animation mixer with the modified result
	...
	reCenter() {
		...
		this.sphereMeshes.children[0].getWorldPosition(
		...
		if (childMesh) {
			...
			this.wordFrames = this.getWorldPositionTimeSeriesByName()
	...
	getWorldPositionTimeSeriesByName() {
	...
	getTimeSeries(jointName = "RightArm") {
		...
		console.log("newParent: ", this.newParent)
		...
		skeleton.bones.forEach((bone, jointIndex) => {
			...
			for (let i = 0; i < rotationTrack.times.length; i++) {
				...
				if (i > 0) {
					...
					let velocity =
						positions[i].distanceTo(positions[i - 1]) / dt // Speed = distance / time
					speed.push(velocity)
	...
	getGestureMetrics(selectedJoints = ["RightHand", "LeftHand"]) {
		...
		for (const joint of allJoints) {
			const { positionsX, positionsY, positionsZ } =
				jointTimeSeries[joint]

			centroid[joint] = {
				x:
					positionsX.reduce((sum, val) => sum + val, 0) /
					positionsX.length,
				y:
					positionsY.reduce((sum, val) => sum + val, 0) /
					positionsY.length,
				z:
					positionsZ.reduce((sum, val) => sum + val, 0) /
					positionsZ.length,
			}
		}
		...
		for (const joint of allJoints) {
			...
			for (let i = 1; i < positionsX.length; i++) {
				...
				const deviation =
					Math.sqrt(dxC * dxC + dyC * dyC + dzC * dzC) / 100
				...
				for (let k = 0; k < i; k++) {
					...
					const diff =
						Math.sqrt(dxK * dxK + dyK * dyK + dzK * dzK) / 100
